...
class PowerWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        # 串口通讯设置区
        comm_group = QGroupBox('设备连接')
        comm_grid = QGridLayout()
        
        # 串口选择
        comm_grid.addWidget(QLabel('串口:'), 0, 0)
        self.port_combo = QComboBox()
        self._update_ports()
        comm_grid.addWidget(self.port_combo, 0, 1)
        
        # 波特率
        comm_grid.addWidget(QLabel('波特率:'), 1, 0)
        self.baud_combo = QComboBox()
        self.baud_combo.addItems(['9600', '19200', '38400', '57600', '115200'])
        self.baud_combo.setCurrentText('9600')
        comm_grid.addWidget(self.baud_combo, 1, 1)
        
        # 地址
        comm_grid.addWidget(QLabel('地址 (1-30):'), 2, 0)
        self.edit_addr = QLineEdit('6' if self.name == '灯丝电源' else ('7' if self.name == '弧电源' else '8'))
        ...
        comm_grid.addWidget(self.edit_addr, 2, 1)
        
        # 连接按钮
        self.btn_connect = QPushButton('连接')
        self.btn_connect.clicked.connect(self._connect_device)
        comm_grid.addWidget(self.btn_connect, 3, 0, 1, 2)
        ...
        self.spin_i.setRange(0, max_current)
        ...
        # 添加电流限制提示
        current_limit_label = QLabel(f'最大电流{max_current}A，超出会设为{max_current}A')
    ...
    def _on_set(self):
        ...
        if i > self.max_current:
            ...
            self.parent().parent().append_log(f"{self.name}: 电流已被限制为{self.max_current}A")
    ...
    def _connect_device(self):
        ...
        if not self._validate_address():
            QMessageBox.warning(self, '地址错误', '请填写有效的设备地址(1-30)')
            return
        ...
        if not port:
            QMessageBox.warning(self, '串口错误', '请选择串口')
            return
        
        try:
            # 打开串口
            ser = serial.Serial(port, baud, timeout=1)
            self._power_supply = TDKPowerSupply(address=addr, serial_connection=ser)
    ...
    def _disconnect_device(self):
        ...
        if self._power_supply:
            ...
            if hasattr(self, 'btn_stop') and self.btn_stop.isEnabled():
                QMessageBox.warning(self, '警告', '请先停止测量再断开连接')
                return
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.left_widget = PowerWidget('灯丝电源', parent=self)
        self.right_widget = PowerWidget('弧电源', parent=self)
        self.third_widget = PowerWidget('偏压电源', parent=self)
        ...
        self.third_widget.set_targets_signal.connect(lambda v, i: self._on_set('电源3', v, i))
        ...
        main_layout.addWidget(self.third_widget, 1, 2)
        main_layout.addWidget(self.log_edit, 2, 0, 1, 3)  # 日志区域跨越三列
    ...
    def power_start_stop(self, name: str, start: bool):
        if start:
            widget = self.left_widget if name == '电源1' else (self.right_widget if name == '电源2' else self.third_widget)
    ...
    def _on_data(self, v, i, p, ts, name):
        ...
        if name == '电源1':
            ...
            self.third_widget.update_outputs(v, i, p)
